...
import { sendWhatsAppNotification, sendCampaignMessage } from "../utils/whatsapp.util.js";
import {
...
router.post("/loyalty/settings", asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, message: "Sadakat ayarları güncellendi", data: business });
}));

router.post("/campaign/send", asyncHandler(async (req, res) => {
  const { campaignText, segment } = req.body;
  const idToUse = req.business_id || req.user?.business_id;

  if (!campaignText) return res.status(400).json({ success: false, message: "Kampanya metni boş olamaz." });

  const business = await Business.findOne({ $or: [{ _id: idToUse }, { business_id: idToUse }] });
  const customers = await Customer.find({ business_id: idToUse.toString() });
  
  if (customers.length === 0) return res.status(404).json({ success: false, message: "Gönderilecek müşteri bulunamadı." });
  ...
  for (const customer of customers) {
    if (customer.phone) {
      try {
        await sendCampaignMessage(customer.phone, campaignText, business?.name, business);
        console.log(`✅ BAŞARILI: ${customer.phone}`);
        successCount++;
      } catch (err) {
        console.error(`❌ HATA (${customer.phone}):`, err.response?.data || err.message);
      }
    }
  }

  console.log(`🏁 KAMPANYA BİTTİ: ${successCount} kişiye ulaştı.\n`);
  res.json({ success: true, message: `${successCount} müşteriye kampanya başarıyla gönderildi.`, totalSent: successCount });
}));
...
router.get("/:businessId/calendar.ics", async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({ business_id: businessId.toString() })
      .populate("service_id")
      .sort({ starts_at: 1 });

    const icsEvents = appointments.map((appt) => {
    ...
    res.set('Content-Disposition', `attachment; filename="${business.name}_calendar.ics"`);
    res.send(icsContent);
  } catch (error) {
